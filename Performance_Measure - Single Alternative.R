...
all_file_paths <- list.files(path = "C22031/", full.names = T)
all_file_names <- list.files(path = "C22031/")
trips <- lapply(all_file_paths, read_excel_allsheets)
names(trips) <- str_sub(all_file_names, end = -6)


### Name convetion for mulriple alternatives specify trip indexes using c(trips[[1]], trips[[6]])
trips_raw <- trips
trips <- trips_raw
number_of_alts <- 1
length_time <- c("LT45" = list(trips$LTime))
...
#Write excel
write_xlsx(trips_ff, "Trips2031C2.xlsx")
write_xlsx(vmt_f, "VMT2031C2.xlsx")
write_xlsx(vht_f, "VHT2031C2.xlsx")
write_xlsx(e_e_f_refined, "EEF2031C2.xlsx")
